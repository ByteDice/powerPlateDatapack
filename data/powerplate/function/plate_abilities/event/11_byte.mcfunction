...
execute as @a[scores = {usedAbility = 1, wearingByte = 1}] at @s at @e[distance = 0.01..30.0, limit = 1, sort = random] run summon armor_stand ~ ~ ~ {Invisible: 1b, Tags: ["bytePlateKillTarget"]}
...
execute as @e[type = armor_stand, nbt = {Tags: ["bytePlateKillTarget"]}] at @s run kill @e[distance = ..0.01]
